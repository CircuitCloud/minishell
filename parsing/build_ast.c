...
/*   Updated: 2024/11/11 03:22:58 by cahaik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_redirection	*init_redir_struct(int type, char *file, char *delimiter, int fd, t_status **p)
{
	t_redirection	*redir;
	redir = malloc(sizeof(t_redirection));
...
t_command	*init_pipe_cmnd(t_ev *ev)
{
	...
	pipe_cmnd->cmnd = ft_strdup("|");
	pipe_cmnd->args = NULL;
	pipe_cmnd->tokens_list->type = PIPE;
	// single_command->args[args_index + 1] = NULL;
	pipe_cmnd->ev = ev;
	// single_command->tokens_list = NULL;
	pipe_cmnd->redir = NULL;
		...
		if (current && current->type == WORD)
		{
			...
			while (current && current->type != PIPE)
			{
				...
				if (current && (current->type == I_RED || current->type == O_RED || current->type == APPEND || current->type == HERDOC))
				{
					...
					if (current && current->type == HERDOC)
						redir_command = init_redir_struct(current->type, NULL, file_or_delim , 0, &p);
					else
						redir_command = init_redir_struct(current->type, file_or_delim , NULL, 0, &p);
					// redir_command = init_redir_struct(current->type, current->next->value,current->next->value, 0);
